...
<body>
	<div id="sellOffice" :class="{'fade':buy==1}">
		<div id="top">
		  <span id="topSpan">
		...
		<div id="sell">
			<div id="sellPic">
				...
				<div :class="{'pshow':p==1}">
					<img src="/img/office1.jpg" alt="企业精选入职体检套餐【男女通用】" width="360px" height="360px">
				</div>
				<div :class="{'pshow':p==2}">
					<img src="/img/office2.jpg" alt="企业精选入职体检套餐【男女通用】" width="360px" height="360px">
				</div>
				<div :class="{'pshow':p==3}">
					<img src="/img/office3.jpg" alt="企业精选入职体检套餐【男女通用】" width="360px" height="360px">
				</div>
				<ul class="clearfix">
					<li>
						<a :class="{'pselected':p==0}" @click.prevent="p=0" href="#"><img src="/img/office.jpg" width="60px" height="60px"/></a>
					</li>
					<li>
						<a :class="{'pselected':p==1}" @click.prevent="p=1" href="#"><img src="/img/office1.jpg" width="60px" height="60px"/></a>
					</li>
					<li>
						<a :class="{'pselected':p==2}" @click.prevent="p=2" href="#"><img src="img/office2.jpg" width="60px" height="60px"/></a>
					</li>
					<li>
						<a :class="{'pselected':p==3}" @click.prevent="p=3" href="#"><img src="img/office3.jpg" width="60px" height="60px"/></a>
					</li>
				</ul>
			</div>
			<form id="sellInfo" action="/addoff" method="post">
				<h2>企业精选入职体检套餐【男女通用】</h2>
				<ul class="clearfix">
					<li>在线订购：<font id="f0">￥320.00元 </font></li>
					<li>商城价格：<s>￥510.00元 </s></li>
					<li>上月订购：2135人</li>
					<li>累计订购：11527人</li>
				</ul>
				<p>
					入职体检是专项体检之一，旨在通过体检保证入职员工的身体状况适合从事该专业工作，在集体生活中不会造成传染病流行，不会因其个人身体原因影响他人。入职体检有相对固定的体检项目与体检标准，选择专业体检中心能保证体检质量。但由于入职体检内容较为简单，所以不能完全代替健康体检。 随着科技的发展，社会不断进步，生活水平的提高，一般单位都要求入职体检。想要入职体检顺利进行，不受其他因素干扰，在体检前做好准备工做是很有必要的.入职体检是企业为了自身生产和集体活动保障绝大多数人健康利益的基本措施，主要是为了防止传染性疾病。 
				</p>
				<div class="selectt">
					<label>预约日期：</label>
				...
				<div id="selectt">
				    <input type="submit" name="submitButton" value="立即购买" @click.prevent="set()"/>
				</div>
			</form>
		</div>
		<div class=" others" id=" others">
			...
			<div :class="{'show':cur==1}">
				<img id="dpic" src="img/detail.jpg">
			</div>
		...
		var userIndex = new Vue({
			...
			methods:{
				set:function(){
					...
					this.$http.post("/addoff",{"date": date}).then(function(res){
						var ss = res.headers.map["sessionstatus"];
						alert( typeof ss );
						if( ss && ss.length > 0){
							ss[0] == timeout;
						}
